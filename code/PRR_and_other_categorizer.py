...
df_wide['CHANGE_OBSERVED'] = df_wide['MOTOR_SCORE_tp2'] - df_wide['INITIAL_IMPAIRMENT'] 
error = 0.9
...
#plt.plot(df_wide['INITIAL_IMPAIRMENT'], df_wide['CHANGE_PREDICTED'] )


#%%
# Define number of iterations
R = 3 # You can adjust this value
...
for cc in range(R):
    ...
    if cc == (R - 1):
        ...
        plt.figure(figsize=(4.5 * cm, 4.5 * cm), dpi=300)
        s = 15
        linewidth = 0.5
    ...
    sns.scatterplot(x='INITIAL_IMPAIRMENT', y='CHANGE_OBSERVED', hue='LINE_CLUSTER', data=df_wide,
                    palette={'HRR': "g", 'PRR': "b", 'outlier': "red"}, marker='o', s=s, alpha=0.7)
    ...
    plt.xlabel('INITIAL_IMPAIRMENT', fontsize=8)
    plt.ylabel('CHANGE_OBSERVED', fontsize=8)
    ...
    if cc == (R - 1):
        ...
        r_squared_df.to_csv(os.path.join(outfile_path, 'r_squared_values.csv'), index=False)
